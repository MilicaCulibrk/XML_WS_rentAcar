      ...
      <v-card hover elevation="2" class="text-center ma-6">
        <div class="cardBorderColor">
          <v-responsive class="pt-4">
            <img
                          :src="addvertisment.images[0].url"
                          height="130px"
                        >
          </v-responsive>
          ...
          >
            ...
            :min="nowDate "
              ...
              <v-btn text color="primary"  @click="reserveDate(dates,addvertisment.id)" >Disable dates</v-btn>
            </v-date-picker>
          </v-dialog>
          </v-card-actions>
        </div>
...
console.log(this.reservedOneDate);
        ...
        reserveDate(dates, id){
            ...
            axios
      .post("/addvertisment-service/reservedDate/" + id, this.createListDates(arr))
      .then(response=> {
    ...
    //izlistavanje oglasa
    axios
      .get("/addvertisment-service/addvertisment/user/" + this.$store.state.user.id )
      .then(addvertisments => {
