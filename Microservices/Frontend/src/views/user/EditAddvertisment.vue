      ...
      <template #activator="{ on: dialogEdit }">
        <v-tooltip bottom color="black">
          <template #activator="{ on: tooltip }">
            <v-btn
              icon
              v-on="{ ...tooltip, ...dialogEdit } "
              @click="setSelectedFields(addvertisment)"
              dark
              fab
          ...
          <v-card-text class="text-center-left">
            <v-form ref="form">
              ...
              <v-row class="mt-n8">
                ...
                <v-col cols="4">
                  <v-text-field label="Location*" color="black" v-model="selectLocation" required></v-text-field>
                </v-col>
                <v-col cols="4">
                  <v-checkbox
...
export default {
  ...
  methods: {
    ...
    setSelectedFields(addvertisment) {
      ...
      axios
        .get("/addvertisment-service/addvertisment/" + this.addvertisment.id)
        .then(addvertismentReal => {
          this.addvertismentReal = addvertismentReal.data;

          this.selectClass.vehicle_class_name =
            addvertisment.vehicle_class_name;
          this.selectClass.id = this.addvertismentReal.vehicle_class_id;
          this.selectTransmission.transmission_type_name =
            addvertisment.transmission_type_name;
